...
Usage:
    ...
    playbook delete [feature | abuser-story | threat-scenario | scan] [--name=<delete-name>]
    playbook configure
...
def get_user_stories(nameval=None):
    """
...
def get_abuser_stories(nameval):
    if not nameval:
...
def get_threat_scenarios(nameval):
    if not nameval:
        print(bad("you need to provide a specific name to query"))
        exit(1)
    query = """
    query {
      searchThreatScenario(name: "%s") {
        name
        vulName
        description
        cwe
...
def delete_abuser_story(nameval):
    data = """
           query {
               mutation {
...
def delete_threat_scenario(nameval):
    data = """
           query {
               mutation {
...
def main():
    ...
    if arguments.get('get'):
        ...
        if arguments.get('--name'):
            ...
            print(bad("You need to specify a name parameter"))
            exit(1)
        ...
        elif arguments.get('abuser-story'):
            get_abuser_stories(name_val)
        elif arguments.get('threat-scenario'):
            get_threat_scenarios(name_val)
        else:
    ...
    if arguments.get('delete'):
        delete_name_val = ""
        if arguments.get('--name'):
            delete_name_val = arguments.get('--name')
        else:
            print(bad("You need to specify a name parameter"))
            exit(1)
        ...
        if arguments.get('feature'):
            if delete_name_val:
                delete_user_story(nameval=delete_name_val)
            elif arguments.get('abuser-story'):
                delete_abuser_story(delete_name_val)
            elif arguments.get('threat-scenario'):
                delete_threat_scenario(delete_name_val)
            else:
                print(bold(red("Unknown Option")))
                exit(1)
